...
export const Notes: React.FC<NoteProps> = ({ note }) => {
  ...
  return (
    ...
    // </div>
      ...
      <div className="relative p-2 transform transition duration-500 hover:scale-110 w-full max-w-xs md:max-w-sm lg:max-w-md xl:max-w-lg">
        <div className="bg-gray-100 dark:bg-gray-700 cursor-pointer text-center shadow-md hover:shadow-lg p-4 rounded-lg w-full" onClick={handleOpenPopup}>
          ...
          <h1 className="font-bold dark:text-gray-300 text-xl uppercase text-[#ffab4a] flex items-center justify-center mt-4">
            <BiNotepad className="mr-2 text-[#ffab4a]" />
            {note.title}
          </h1>
          <p className="dark:text-gray-300 ml-5">{note.content}</p>
        </div>
      </div>
