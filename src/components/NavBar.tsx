...
const NavBar = () => {
  const { searchString, setSearchString, fetchNotes } = useNotesStore();
      ...
      <div className="p-4">
        <div className="flex flex-col sm:flex-row items-center">
          ...
          <div className="sm:hidden w-full mt-4">
            <Input type="search" placeholder="Search" value={searchString} onChange={(e) => { setSearchString(e.target.value); }} className="w-full" />
          </div>

          {/* Hidden on mobile, visible on larger screens */}
          <div className="hidden sm:flex flex-1 justify-start items-start mt-4 sm:mt-0">
            <Input type="search" placeholder="Search" className="w-full max-w-md" value={searchString} onChange={(e) => { setSearchString(e.target.value); }} />
          </div>
